function Draggable (element, handle) {
  ...
  function init (element, handle) {
    handle = handle || element
    var index = parseInt(getStyle(element, 'z-index'), 10)
    fairlyHighZIndex = isNaN(index) ? '10' : index
    setPositionType(element)
    setDraggableListeners(element)
    addEventListener(handle, 'mousedown', (event) => {
      let button = null
  ...
  function getInitialPosition (element) {
    ...
    if (getStyle(element, 'position') === 'absolute') {
      rect = {
        top: parseInt(getStyle(element, 'top'), 10),
        left: parseInt(getStyle(element, 'left'), 10)
      }
    } else {
