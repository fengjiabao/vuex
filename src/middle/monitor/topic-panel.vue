...
export default {
  ...
  methods: {
    ...
    formatDetailData (cards) {
      ...
      if (self.topic.type === OD.VEHICLE) {
        for (let i = 0, len = cards.length; i < len; i++) {
          let row = []
          let card = cards[i]
          let stateDef = stateDefs['vehicle']
    ...
    isSpecial (id) {
      ...
      let areaTypeID = null
      if (id !== 0) {
        areaTypeID = this.$store.metaStore.data.area.get(id) && this.$store.metaStore.data.area.get(id).area_type_id
      }
